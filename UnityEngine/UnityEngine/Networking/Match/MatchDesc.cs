namespace UnityEngine.Networking.Match
{
    ...
    internal class MatchDesc : ResponseBase
    {
        public override void Parse(object obj)
        {
            ...
            this.averageEloScore = base.ParseJSONInt32("averageEloScore", obj, dictJsonObj);
            ...
            this.hostNodeId = (NodeID) base.ParseJSONUInt16("hostNodeId", obj, dictJsonObj);
            this.directConnectInfos = base.ParseJSONList<MatchDirectConnectInfo>("directConnectInfos", obj, dictJsonObj);
        }

        public override string ToString()
        {
            object[] args = new object[] { base.ToString(), this.networkId.ToString("X"), this.name, this.averageEloScore, this.maxSize, this.currentSize, this.isPrivate, (this.matchAttributes != null) ? this.matchAttributes.Count : 0, this.hostNodeId, this.directConnectInfos.Count };
            return UnityString.Format("[{0}]-networkId:0x{1},name:{2},averageEloScore:{3},maxSize:{4},currentSize:{5},isPrivate:{6},matchAttributes.Count:{7},hostNodeId:{8},directConnectInfos.Count:{9}", args);
        }
